...
async function searchLocation() {
    ...
    if (location) {
        const apiGeoLocation = `http://api.openweathermap.org/geo/1.0/direct?q=${location}&limit=1&appid=${apiKey}`;
...
function userLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
        ...
        async position => {
            ...
            const reverseGeoLocation = `http://api.openweathermap.org/geo/1.0/reverse?lat=${latitude}&lon=${longitude}&limit=1&appid=${apiKey}`;
...
async function getCoordinates(name, lat, lon) {
    ...
    const apiWeatherInfo = `http://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&appid=${apiKey}`;
