...
async function searchLocation() {
    ...
    if (location) {
        const apiGeoLocation = `https://crossorigin.me/api.openweathermap.org/geo/1.0/direct?q=${location}&limit=1&appid=${apiKey}`;
...
function userLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
        ...
        async position => {
            ...
            const reverseGeoLocation = `https://crossorigin.me/api.openweathermap.org/geo/1.0/reverse?lat=${latitude}&lon=${longitude}&limit=1&appid=${apiKey}`;
...
async function getCoordinates(name, lat, lon) {
    ...
    const apiWeatherInfo = `https://crossorigin.me/api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&appid=${apiKey}`;
        ...
        if (index === 0) {
            return `<div class="weather__details" id="weather__details">
                    ...
                    <div class="weather__icon" id="weather__icon">
                        <img src="https://crossorigin.me/openweathermap.org/img/wn/${weatherItem.weather[0].icon}@4x.png" alt="${weatherItem.weather[0].description} Icon">
                        <p>${weatherItem.weather[0].description}</p>`
        } else {
            return `<div class="card">
                        <h2 id="${index}">(${weatherItem.dt_txt.split(" ")[0]})</h2>
                        <img src="https://crossorigin.me/openweathermap.org/img/wn/${weatherItem.weather[0].icon}@2x.png" alt="${weatherItem.weather[0].description} Icon">
